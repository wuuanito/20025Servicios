...
const OrdenFabricacion = sequelize.define('OrdenFabricacion', {
  ...
  tiempoEstimadoProduccion: {
    ...
    allowNull: true,
    comment: 'Tiempo estimado en minutos = unidades a producir / estándar teórico (16.6)'
  },
  ...
  standardRealVsTeorico: {
    ...
    allowNull: true,
    defaultValue: null,
    comment: '(standardReal - 16.6) / 16.6'
  },
  ...
  rendimiento: {
    ...
    allowNull: true,
    defaultValue: null,
    comment: 'total unidades / (tiempo activo * estándar teórico 16.6) - valor entre 0 y 1'
  },
  calidad: {
    type: DataTypes.FLOAT,
    allowNull: true,
  ...
  orden.horaInicio = DateTime.now().setZone('Europe/Madrid').toJSDate();
      ...
      try {
        ...
        if (orden.cantidadProducir && !orden.tiempoEstimadoProduccion) {
          orden.tiempoEstimadoProduccion = orden.cantidadProducir / 16.6;
        }
