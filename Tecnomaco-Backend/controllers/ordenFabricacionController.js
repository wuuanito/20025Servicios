...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const datosOrden = {
      ...
      estado: 'creada',
      tiempoEstimadoProduccion: parseInt(req.body.cantidadProducir) / 16.6
    };
...
const calcularTiempoPausasQueComputan = async (ordenFabricacionId, transaction = null) => {
  ...
  for (const pausa of pausas) {
    // Solo sumar si la pausa computa en tiempo (no es cambio_turno, pausa_parcial ni fin_jornada)
...
exports.finalizar = async (req, res) => {
  ...
  try {
    ...
    const tiempoEstimadoProduccion = ordenF.cantidadProducir / 16.6;
